...
/// struct for typed errors of method [`get_net_worth_api_net_worth_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetNetWorthApiNetWorthGetError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}


pub async fn get_net_worth_api_net_worth_get(configuration: &configuration::Configuration, period: Option<&str>) -> Result<std::collections::HashMap<String, serde_json::Value>, Error<GetNetWorthApiNetWorthGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_period = period;

    let uri_str = format!("{}/api/net-worth/", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetNetWorthApiNetWorthGetError> = serde_json::from_str(&content).ok();
